...
// Text generator with chatgpt, used from Fmenu
export default function AiText({ editor, setVisible }: { editor: any; setVisible: any }) {
  ...
  async function onGenerate() {
    setLoading(true);
    try {
      const res = await api.post(`/user/text_generate?q=${value}`, {}, {timeout: 1000*30}); // make timeout of 30 seconds
    ...
    <div className="bg-black/30 top-0 left-0 fixed w-[100vw] h-[100vh] flex items-center justify-center z-[3000000]">
      <div className="w-[500px] rounded-xl shadow-lg p-5 flex flex-col bg-white" ref={ref} >
        <textarea
          ...
          className="h-[100px] text-xl mb-5 resize-none outline-none"
        />
        <button
          onClick={onGenerate}
          type="button"
